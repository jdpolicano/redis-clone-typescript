...
export default class Replconf extends Command {
    ...
    public execute(): Transaction {
        ...
        if (this.options.getAck) {
            const args = ["REPLCONF", "ACK", this.ctx.serverInfo.getMasterReplOffset().toString()];
            this.reply(() => this.ctx.connection.writeResp(RespBuilder.bulkStringArray(args)));
        }
    ...
    static parseArgs(args: RespBulkString[]): ReplconfOptions {
        ...
        if (firstArg === "listening-port") {
            if (!args[1]?.value) {
                throw new Error("ERR wrong number of arguments for 'replconf listening-port' command");
            }
            ...
            if (!args[1]?.value) {
                throw new Error("ERR wrong number of arguments for 'replconf getack' command");
            }
