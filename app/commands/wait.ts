...
export default class Wait extends Command {
  ...
  static parseWaitOptions(args: RespBulkString[]): WaitOptions {
    ...
    const maxWaitTimeInt = maxWaitTime
      ? parseInt(maxWaitTime.value as string, 10)
      : 0;
  ...
  public async execute(): Promise<Transaction> {
    ...
    if (currStreamIdx === 0) {
      const numConnectedReplicas =
        this.ctx.replicationStream.getReplicas().length;
      this.reply(() =>
        this.ctx.connection.writeResp(
          RespBuilder.integer(numConnectedReplicas.toString()),
        ),
      );
    ...
    this.ctx.replicationStream.replicate(
      Buffer.from(payload),
      this.ctx.serverInfo,
    );
    ...
    await new Promise(async (resolve) => {
      ...
      while (Date.now() - executionStart < maxWaitTime) {
        ...
        const countGreaterThanWaitParam = replicas.filter(
          (replica) => replica.getLastKnownOffset() >= currStreamIdx,
        ).length;
        maxAcks = Math.max(maxAcks, countGreaterThanWaitParam);
        if (maxAcks >= numClients) {
          this.reply(() =>
            this.ctx.connection.writeResp(
              RespBuilder.integer(maxAcks.toString()),
            ),
          );
          resolve(void 0);
          return;
      ...
      this.reply(() =>
        this.ctx.connection.writeResp(RespBuilder.integer(maxAcks.toString())),
      );
      resolve(void 0);
    });
